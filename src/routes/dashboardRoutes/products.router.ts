...
import { createProductSchema, updateProductSchema, getProductSchema, getProductsFilterSchema } from '../../schemas/product.schema'
import validatorHandler from '../../middlewares/validator.handler'
import { handleResponse } from '../../responses/response'

const router:Router = express.Router();

router.get('/', passport.authenticate('jwt', {session:false}), validatorHandler(getProductsFilterSchema, 'query'), async(req:Request, res:Response, next:NextFunction) => {
  try {
    const merchantId = req.user?.sub as number
    const filter = req.query
    const products = await productService.get(merchantId, filter);
